...
def index():
    ...
    health_status = {
        ...
        "components": {
            ...
            "model": MODEL_LOADED,
            "mode": "production" if (PREPROCESS_AVAILABLE and MODEL_LOADED) else f"Model Load Failure: {e}"
        }
    }
...
def debug_status():
    return {
        ...
        "model_object": str(type(eeg_model)) if eeg_model else f"Model Load Failure: {e}"
    }
...
async def predict_eeg(file: UploadFile = File(...)):
    ...
    if PREPROCESS_AVAILABLE and preprocess_eeg:
        try:
            ...
            result = create_dummy_response(file.filename, f"preprocessing_error: {str(e)}")
            return result
    ...
    if MODEL_LOADED and eeg_model:
        try:
            ...
            result = create_dummy_response(file.filename, f"prediction_error: {str(e)}")
            return result
